...
│   ├── quoting-engine/              # Avellaneda-Stoikov/GLFT closed-form adapted to DV01 inventory + real maker rate bounds, zero-maker-fee spread economics
...
Risk management:         ██████████████████  95%  (margin-sim + risk-engine closed against real formulas; only the live monitoring process itself — services/risk-monitor, Sprint 4 — remains)
...
Strategy:                ████████████████░  95%  (curve-engine, quoting-engine, arb-engine all done and tested; DV01-neutral sizing for calendar spreads intentionally left to the caller)
...
**`crates/curve-engine`** — first Sprint 3 piece; not verified against protocol source because there's no protocol source for this — Boros doesn't publish or require a term structure (each market's implied APR is independently discovered by its own orderbook + AMM, `whitepapers/AMM.pdf`). This is a strategy tool, grounded in the standard curve-construction literature instead (Fritsch & Carlson 1980; Hagan & West 2006) rather than invented ad hoc
- `MonotoneCubicSpline`: Fritsch-Carlson monotone cubic Hermite interpolation, f64 throughout (a reference/signal curve, not a settlement calculation — same precedent as `tick_to_rate`/`rate_to_tick`). Never extrapolates past the observed maturity range
- `Curve`: fits a spline per `Zone` (Boros's own cross-margin grouping — same underlying, multiple maturities), `rate_at()` for an interpolated reference rate, `detect_butterflies()` for relative-value signals
- Deliberately named **signal**, not arbitrage: unlike a bond curve (where a negative butterfly is a riskless, replicable position), Boros markets at different maturities aren't fungible on-chain — there's no mechanism to convert exposure from one maturity to another directly. A detected butterfly is a candidate calendar spread, not guaranteed profit; documented explicitly so nothing downstream treats it as more than it is
- 17 tests, 0 warnings — including no-overshoot checks on both monotone data and a local extremum (the actual failure mode a naive cubic spline has and this algorithm exists to avoid), and sign-correctness on both a "cheap" and a "rich" middle maturity

**`crates/quoting-engine`** — Avellaneda-Stoikov (2008) / Guéant-Lehalle-Fernandez-Tapia (2012) closed form, adapted to DV01 inventory and rate space; three facts confirmed against the real Boros developer docs (`docs.pendle.finance/boros-dev`), not assumed
- `reservation_rate`/`optimal_spread`: classic A-S skew and spread, plus an honestly-scoped `carry_adjustment` term — inspired by, but explicitly weaker than, the `-qf` funding/inventory coupling in *"Funding-Aware Optimal Market Making for Perpetual DEXs"* (arXiv:2605.06405), because Boros pays its fixed leg **upfront** (not a running cost like perpetual funding), so the paper's coupling doesn't transfer as cleanly — documented in-code rather than overclaimed. `carry_weight = 0.0` disables it
- **Real finding, not obvious from the formula**: the two terms of `optimal_spread` don't move the same direction in `γ` — the liquidity term can *shrink* as risk-aversion increases for short horizons/low vol (the common case for frequent requoting), so "higher `γ` always widens the spread" is false in general. Documented in the function's own doc comment, and covered by two tests exercising both regimes (inventory-dominated: widens with `γ`; liquidity-dominated: narrows with `γ`) — found by a test that initially asserted the naive monotonic assumption and failed
- `MakerRateBounds`/`clamp_rate`: exact port of the real maker rate bound formula (`Upper/Lower Bound = MarkRate × (1 ± (loConst + loSlope × TTM))`, source: `Mechanics/OrderBook.md#restrictions`) — a quote outside these bounds is a guaranteed on-chain revert (`Limit Rate Out of Bounds`), not a soft suggestion
- Spread economics confirmed **zero maker fee** (`Mechanics/Fees.md`: "Maker orders incur no fees when placed") — this crate's spread doesn't add a fee-coverage floor, since as a market maker every resting quote is a maker fill
- No calibration defaults anywhere (`gamma`/`sigma`/`kappa`/`horizon_secs`/`carry_weight` all required, validated at construction) — these are real trading parameters a desk calibrates, not something this crate should guess
- 21 tests, 0 warnings

**`crates/risk-engine`** — pre-trade limits, runtime divergence monitoring, kill switch — built on `margin-sim`'s already-verified `MarginEngine` rather than re-deriving margin math
- `check_pre_trade`: DV01 (net + gross, via a new shared `position_dv01` matching the `DV01 = |size| × ttm_years × 0.0001` formula this workspace has used conceptually since before this crate existed), notional, order-rate throttle, and projected health-ratio-after-order (delegated straight to `MarginEngine::compute_account_state`) — returns **every** violation found, not just the first
- `monitor`: shadow-vs-real divergence checks for health ratio and mark rate, both/neither-infinite handled explicitly (a flat account on both sides isn't a divergence)
- `KillSwitch`: latches on trip, requires an explicit reasoned `reset()` — never silently self-heals. Doesn't and can't enforce process isolation on its own (a struct can't); that's `services/risk-monitor`'s job (Sprint 4) — this crate provides the logic both processes would share
- No defaults for any limit or tolerance — real risk policy, not this crate's to guess
- 18 tests, 0 warnings
...
- `detect_cross_venue_signal`: Boros implied APR vs. a comparable CEX perp's expected funding — basis risk is real, "expected" CEX funding isn't locked in the way Boros's fixed leg is, and the two floating indices aren't identical by construction
- `to_calendar_spread_trade`: wraps `curve_engine::ButterflySignal` into a directional trade (opposite sides at mid vs. wings) — deliberately not DV01-sized, since a true risk-minimized butterfly needs position-sizing context (account size, risk budget) this crate doesn't have; gives direction, not size
- 7 tests, 0 warnings
