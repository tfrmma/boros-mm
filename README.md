...
Risk management:         █████████░░░░░░░  55%  (margin-sim done with one open precision gap; risk-engine pending)
...
- 37 unit tests + 2 golden-vector tests, 0 warnings
...
- Known open gap (documented in-code, not fixed): `Position::value()` computes PV via two chained `mul_fixed` calls (double rounding); the real contract's `calcPositionValue` is one triple product then one `rawDivFloor` (single rounding), not numerically identical even with exact arithmetic on both sides. See technical debt table.
...
|---|---|---|
| `margin-sim/types.rs` | `Position::value()` double-rounds (two chained `mul_fixed`) vs. the contract's single fused `rawDivFloor(size*rate*ttm, ONE*YEAR)` | Small raw-unit divergence from on-chain PV even with exact arithmetic; found while verifying settlement-ledger, documented in-code, not yet fixed |
...
| `margin-sim/types.rs` | `HealthThresholds` (risky/liquidation/deleverage) unconfirmed against contract | The real `IM`/`MM`/`hcrit` formulas are now known (ChainSecurity audit of `boros-core-public`) but not yet wired in |
...
- Resolve `Position::value()` double-rounding vs. the contract's fused single-division formula, exact `open_order_im` netting, live wire-format verification for `boros/types.rs`
