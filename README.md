...
Strategy:                █████░░░░░░░░░░░  30%  (curve-engine + quoting-engine done and tested; arb-engine pending it's the natural consumer of curve-engine's butterfly signal, not yet wired up)
...
**3: strategy**
- `arb-engine`: cash-and-carry cross-venue (Boros implied APR vs realized CEX funding) + relative-value cross-maturity, consuming `curve-engine`'s butterfly signal directly
- `risk-engine`: pre-trade DV01/notional/health-ratio limits, mark-rate divergence detection, independent kill-switch authority can reuse `margin-sim`'s `MarginViewUtils`/`LiquidationViewUtils` ports directly
